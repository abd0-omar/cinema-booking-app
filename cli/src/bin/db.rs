        ...
        Ok(config) => {
            match cli.command {
                Commands::Drop => {
                    ...
                    let db_path = drop(&config.database)
                        .await
                        .context("Could not drop database!")?;
                    ui.success(&format!("Dropped database {} successfully.", db_path));
                    Ok(())
                }
                Commands::Create => {
                    ui.info(&format!("Creating {} database…", &cli.env));
                    let db_path = create(&config.database)
                        .await
                        .context("Could not create database!")?;
                    ui.success(&format!("Created database {} successfully.", db_path));
                    Ok(())
                }
                Commands::Migrate => {
                ...
                Commands::Reset => {
                    ...
                    ui.outdent();
                    let db_path = result?;
                    ui.success(&format!("Reset database {} successfully.", db_path));
                    Ok(())
                }
                Commands::Prepare => {
                    ...
                    // Convert relative database path to absolute for sqlx prepare
                    // since it runs from the db crate directory
                    let database_url = resolve_database_url(&config.database.url)?;
                    ...
                    if !o.status.success() {
                        ...
                        let error = anyhow!("{}\n{}", stdout, stderr).context("Error generating query metadata. Are you sure the database is running and all migrations are applied?");
                        return Err(error);
                    }
...
/// Get the database file path from the config URL
fn get_db_path(config: &DatabaseConfig) -> Result<PathBuf, anyhow::Error> {
    let url = &config.url;
    // Parse sqlite:path?mode=rwc format
    let path_str = url
        .strip_prefix("sqlite:")
        .ok_or_else(|| anyhow!("Database URL must start with 'sqlite:'"))?;
...
/// Resolve a database URL with a relative path to use an absolute path.
/// This is needed for commands that run from a different directory (like sqlx prepare).
fn resolve_database_url(url: &str) -> Result<String, anyhow::Error> {
    // Parse sqlite:path?mode=rwc format
    ...
    let (path_str, query) = match path_and_query.split_once('?') {
        Some((p, q)) => (p, Some(q)),
    ...
    // Rebuild the URL with absolute path
    let abs_url = match query {
        Some(q) => format!("sqlite:{}?{}", abs_path.display(), q),
        None => format!("sqlite:{}", abs_path.display()),
    };
...
async fn create(config: &DatabaseConfig) -> Result<String, anyhow::Error> {
    ...
    // Create the database with optimal PRAGMA settings via connect options
    ...
    let _connection: SqliteConnection = Connection::connect_with(&options)
        .await
...
async fn migrate(ui: &mut UI<'_>, config: &DatabaseConfig) -> Result<i32, anyhow::Error> {
    let options = get_db_options(config)?;
...
async fn reset(ui: &mut UI<'_>, config: &DatabaseConfig) -> Result<String, anyhow::Error> {
    ...
    let db_path = create(config).await?;
...
fn get_db_options(config: &DatabaseConfig) -> Result<SqliteConnectOptions, anyhow::Error> {
    SqliteConnectOptions::from_str(&config.url).context("Invalid DATABASE_URL!")
}

async fn get_db_client(config: &DatabaseConfig) -> Result<SqliteConnection, anyhow::Error> {
    let options = get_db_options(config)?;
    let connection: SqliteConnection = Connection::connect_with(&options)
        .await
        .context("Failed to connect to database!")?;
