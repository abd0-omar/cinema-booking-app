...
async fn generate_middleware(name: String, r#override: bool) -> Result<String, anyhow::Error> {
    ...
    append_module_definition_to_project_file("./web/src/middlewares/mod.rs", &name, true)?;
...
async fn generate_controller(name: String, r#override: bool) -> Result<String, anyhow::Error> {
    ...
    create_project_file(&file_path, output.as_bytes(), r#override)?;
    append_module_definition_to_project_file("./web/src/controllers/mod.rs", &name, true)?;
...
async fn generate_entity(
    ...
    append_module_definition_to_project_file("./db/src/entities/mod.rs", &name_plural, true)?;

    Ok(struct_name)
}

async fn generate_entity_test_helper(
    name: String,
    r#override: bool,
) -> Result<String, anyhow::Error> {
    let name = to_singular(&name).to_lowercase();
    ...
    append_module_definition_to_project_file("./db/src/test_helpers/mod.rs", &name_plural, true)?;
...
async fn generate_crud_controller_test(
    name: String,
    r#override: bool,
) -> Result<String, anyhow::Error> {
    let name = to_snake_case(&name).to_lowercase();
...
fn create_project_file(path: &str, contents: &[u8], r#override: bool) -> Result<(), anyhow::Error> {
    if !r#override && Path::new(path).exists() {
        ...
        let mut file =
            File::create(path).context(format!(r#"Could not create file "{}""#, path))?;
...
fn append_module_definition_to_project_file(
    path: &str,
    module_name: &str,
    is_public: bool,
) -> Result<(), anyhow::Error> {
