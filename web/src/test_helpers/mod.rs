...
use cinema_booking_db::test_helpers::{setup_db, teardown_db};
...
///
/// A `DbTestContext` is passed as an argument to tests marked with the [`cinema_booking_macros::db_test`] attribute macro. It is used to access the application under test as well as the database (which is the same database the application under test uses).
///
/// Example:
/// ```
/// #[db_test]
